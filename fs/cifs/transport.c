		...
		const struct cifs_credits *exist_credits)
{
	...
	struct mid_q_entry *mid;
	struct cifs_credits credits_on_stack = { .value = 0, .instance = 0 };
		   ...
		   int *resp_buf_type, struct kvec *resp_iov)
{
	...
	for (i = 0; i < num_rqst; i++) {
		...
		/*
		 * All parts of the compound chain must get credits from the
		 * same session, otherwise we end up using more credits than
		 * the server granted. If there were reconnects in between,
		 * return -EAGAIN and let callers handle it.
		 */
		if (rc == 0 && i > 0 && first_instance != instance) {
			i++;
			rc = -EAGAIN;
		}
	 ...
	 * the srv_mutex. We can't use credits obtained from the previous session
	 * to send this request. Check if there were reconnects after we obtained
	 * credits and return -EAGAIN in such cases to let callers handle it.
	 */
